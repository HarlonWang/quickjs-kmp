...
**字节码是引擎序列化加一个头。** `kmpjs_compile` 在临时 Runtime 里 `JS_Eval(COMPILE_ONLY)` 再 `JS_WriteObject`，前置 52 字节头（magic、种类、上游 commit）；`kmpjs_run_bytecode` 核对头后 `JS_ReadObject`，脚本直接 `JS_EvalFunction`，模块先 `JS_ResolveModule` 再走与 `kmpjs_eval_module` 相同的收尾。字节码模块按编译时的名字注册，因为读取即入缓存、名字读取后才知道。
