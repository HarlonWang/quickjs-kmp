...
脚本与模块都经 `JS_Eval(COMPILE_ONLY)` → `JS_WriteObject(JS_WRITE_OBJ_BYTECODE)`，编译在一个临时 Runtime 里完成、不需要引擎；QuickJS 在 COMPILE_ONLY 下仍会解析模块的 import，所以编译运行时挂一个对任何名字都返回空 C 模块的占位 loader，字节码里只记依赖的名字。加载走 `JS_ReadObject` + `JS_EvalFunction`，模块先 `JS_ResolveModule`。文件头只记 magic、种类（脚本 / 模块）与上游 commit：QuickJS 自带 BC 版本号但不校验 commit。不记字长：QuickJS 的序列化全部是 leb128 或定宽小端，BigInt limb 虽按 `JS_LIMB_BITS` 分 32 / 64 位写但字节序列相同，唯一写指针的 `JS_WRITE_OBJ_SAB` 不开，所以同一份字节码 arm64 与 armeabi-v7a 通用（qjsc 只有字节序开关、没有字长开关，同一事实）。`JsBytecode.wordSize` 与 `compile` 的 `wordSize` 参数不存在。调试信息裁剪对应 QuickJS 的 Runtime 级 `JS_SetStripInfo`，语义是「去源码」或「去全部调试信息」而非「去列号」，`compile` 的参数为 `strip: JsBytecode.Strip`（NONE / SOURCE / DEBUG）。「每引擎一个程序」「加载必须先于求值」的限制不复存在，可以多次加载、任意时机加载：`JsEngine.runBytecode(bytes)` 对脚本给完成值、对模块给 namespace，与 `evaluate` / `evaluateModule` 同形。字节码模块经 `registerModule(bytes)` 注册，**名字取自字节码里编译时的名字、不由调用方指定**：读取 `JS_ReadObject` 之前无从得知名字，而读取就已经把模块定义放进了 context 的缓存且无法移除，若允许调用方传名再校验，「名字不匹配」的拒绝会留下一份排在最前、抢占该名字的残留定义；改为按编译名注册后，残留只在「名字已被占用」时出现，此时先注册的定义仍排在前面，残留无害。匿名（尖括号）名字的字节码模块拒绝注册。宿主编译工具 `qjsc-kmp`（`buildHostTools`）与 `JsBytecode.compile` 共用 shim 的编译入口。
