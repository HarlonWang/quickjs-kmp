...
`registerModule(name, source)` 把模块源码登记进 C 侧名字表，第一次被 import 时才编译（字节码形态在 M3 作为重载加入，C 侧接口只换编译入口）；`JS_SetModuleLoaderFunc` 的 loader 只查表，查不到抛 `ReferenceError: module 'x' is not registered`；normalize 不处理相对路径（裸说明符原样返回），`import './x.js'` 同样按字面量查表失败。直接求值过的模块同样以它的名字进了 context 的缓存，之后可以被 import，所以注册名与求值名是同一个命名空间，任何一个名字只能占用一次，重复一律报错：QuickJS 在第一次 import 后把模块缓存在 context 里，之后换源码对引擎无效，静默失效不如拒绝；尖括号形式的名字（默认的 `<module>`）视为匿名，不入表、不可 import。`evaluateModule(source, name)` 先 COMPILE_ONLY 拿到 `JSModuleDef` 再 `JS_EvalFunction`，模块求值返回 Promise（顶层 await），走统一的排空与解包：fulfilled 时返回 `JS_GetModuleNamespace` 的 namespace，一律以 ref 交出（活对象，调用方要 `get("default")` 再 call）；rejected 抛异常；pending 返回带 `isPromise` 的 ref。不做文件系统 loader、不做相对路径解析——构建工具负责让模块图里只剩表内名字。表外的名字由 `JsEngineConfig.moduleLoader` 补：解析顺序是名字表 → loader → `ReferenceError`，loader 返回 `JsModuleSource.Text` / `Bytecode` / `null`，抛异常则 import 处抛 `Error`；给出的模块入 context 缓存并占名（同名不再问、同名注册报错），`null` 与失败不占名、下次 import 再问——热下发的缓存可能晚到，负缓存会把它永久挡在外面；字节码的编译名必须等于请求名，否则拒绝。**loader 必须同步返回、不得在回调里调引擎**：QuickJS 的 `JSModuleLoaderFunc` 是同步的，静态 import 与动态 `import()` 都在引擎线程上、在发起 import 的那次调用内调它，没有"稍后 resolve"的余地；回调里再进引擎（注册别的名字、求值）会在模块图解析中途改名字表。热下发因此是宿主侧的事：JS 经 J 入口请求预取 → Kotlin 下载完回调 K 入口 → JS 再 `import()`，loader 只查缓存。模块名即说明符，`import.meta.url` 为 `<scheme>:<name>`，scheme 由 `JsEngineConfig.moduleScheme` 指定，默认 `kmp`。已知代价：QuickJS 把编译过的模块定义挂在 context 上直到引擎销毁，`evaluateModule` 每调一次都留下一份，反复求值同一段源码会慢涨内存；TinyUI 每页一个 Runtime、页面模块只求值一次，不受影响，通用用户反复跑的东西应该用 `evaluate` 或注册成模块后 import。`JsRuntime` 只包装 `registerModule`，`evaluateModule` 返回的 ref 必须在 `withEngine` 里用，不给锁外入口。

## 字节码：QuickJS 序列化 + 自家文件头，字长无关

脚本与模块都经 `JS_Eval(COMPILE_ONLY)` → `JS_WriteObject(JS_WRITE_OBJ_BYTECODE)`，编译在一个临时 Runtime 里完成、不需要引擎；QuickJS 在 COMPILE_ONLY 下仍会解析模块的 import，所以编译运行时挂一个对任何名字都返回空 C 模块的占位 loader，字节码里只记依赖的名字。加载走 `JS_ReadObject` + `JS_EvalFunction`，模块先 `JS_ResolveModule`。文件头只记 magic、种类（脚本 / 模块）与上游 commit：QuickJS 自带 BC 版本号但不校验 commit。不记字长：QuickJS 的序列化全部是 leb128 或定宽小端，BigInt limb 虽按 `JS_LIMB_BITS` 分 32 / 64 位写但字节序列相同，唯一写指针的 `JS_WRITE_OBJ_SAB` 不开，所以同一份字节码 arm64 与 armeabi-v7a 通用（qjsc 只有字节序开关、没有字长开关，同一事实）。`JsBytecode.wordSize` 与 `compile` 的 `wordSize` 参数不存在。调试信息裁剪对应 QuickJS 的 Runtime 级 `JS_SetStripInfo`，语义是「去源码」或「去全部调试信息」而非「去列号」，`compile` 的参数为 `strip: JsBytecode.Strip`（NONE / SOURCE / DEBUG）。「每引擎一个程序」「加载必须先于求值」的限制不复存在，可以多次加载、任意时机加载：`JsEngine.runBytecode(bytes)` 对脚本给完成值、对模块给 namespace，与 `evaluate` / `evaluateModule` 同形。字节码模块经 `registerModule(bytes)` 注册，**名字取自字节码里编译时的名字、不由调用方指定**，一个模块只有一个名字，没有「传名再校验」这条会失败的路。`JS_ReadObject` 一读就把模块定义放进 context 缓存且无法移除，而源码模块在第一次 import 前并不在缓存里，被拒绝的读取会留下一份抢在源码前面的定义；所以名字查重在读入引擎之前完成：先在一个 `JS_NewContextRaw` 的裸 context 里读一遍取名字，查重通过再读进真正的 context，裸 context 随即释放。匿名（尖括号）名字的字节码模块拒绝注册。宿主编译工具 `qjsc-kmp`（`buildHostTools`）与 `JsBytecode.compile` 共用 shim 的编译入口。
